...
class IntentsView(tk.Frame):
    def __init__(self, parent):
        super().__init__(parent, bg="#ECECEC")
        tk.Label(self, text="📄 Vista de Intents", font=("Segoe UI", 18), bg="#ECECEC").pack(pady=10)
    ...
    def cargar_datos(self):
        ...
        config = load_config()
        csv_path = config.get("csv_intents")
        if not csv_path:
            messagebox.showerror("Error", "Ruta del CSV de intents no definida.")
            return
        csv_path = resolve_data_path(csv_path)
        if not os.path.exists(csv_path):
            messagebox.showerror("Error", "Ruta del CSV de intents no válida o no definida.")
            return

        try:
            df = pd.read_csv(csv_path)
            # Aplicar forward-fill a columnas clave
            if "Intent Display Name" in df.columns:
                df["Intent Display Name"] = df["Intent Display Name"].ffill()
            if "Language" in df.columns:
                df["Language"] = df["Language"].ffill()
